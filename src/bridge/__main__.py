...
class Harmony:
    """The main class for Harmony bridge."""

    def __init__(self, bot, host_platform, host_platform_obj):
        self.__host = bot # host bot object
        self.__data = AutoSaveDict()  # this should be kept private to prevent tampering
        self.__platforms = Platforms() # platforms list
        self.__host_platform = host_platform # host platform
        self.__host_public = Host(self, self.__host, host_platform) # host interface

        # add host platform
        self.__platforms.add_platform(self.__host_platform, host_platform_obj)
        self.__platforms.enable_platform(self.__host_platform) # this should always be enabled
    ...
    async def send_bridge(self, target: messageable.Messageable, content, user: User = None, specials: MessageFeatures = None):
